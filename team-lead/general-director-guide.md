...
> **使用顺序**：
> 1. 先读 `team-lead/project-state.md` 获取当前真实状态
> 2. 再读本文件决定如何分派和审核任务
> 3. 如果需要维护文档体系，转到 `team-lead/documentation-architect-guide.md`
...
一个**中英双语攀岩知识库网站**，内容分为三个独立支柱：

| 支柱 | 定位 | 框架文件 |
|------|------|---------|
| **支柱一：知识库** | 对攀岩表现有直接帮助的概念、技术、原理 | `climbing-knowledge-framework.md` |
| **支柱二：训练手册** | 知识的落地执行方案（具体训练卡，可直接用作训练菜单） | `training-framework.md` |
| **支柱三：攀岩名人** | 运动员生平、训练理念、采访（人物文化内容） | `athlete-framework.md` |

> 三支柱判断原则：「不知道这个内容会影响攀爬表现」→ 知识库；「是知识的执行落地」→ 训练手册；「人物/文化/历史」→ 攀岩名人。

**当前项目规模、目录和进度以 `team-lead/project-state.md` 为准。**
...
```
rock-climbing/
├── 📐 team-lead/                        ← 内容架构与总负责人文件
│   ├── general-director-guide.md        → 本文件
│   ├── project-state.md                 → 当前项目状态真源
│   ├── documentation-architect-guide.md → 文档体系治理
│   ├── climbing-knowledge-framework.md  → 支柱一：知识库架构（10 大板块）
│   ├── training-framework.md            → 支柱二：训练手册架构
│   └── athlete-framework.md             → 支柱三：攀岩名人档案架构
│
├── 📋 各角色 Guide（分布在各角色文件夹）
│   ├── video-researcher/video-research-guide.md     → 视频搜索 bot
│   ├── article-researcher/article-research-guide.md → 文章搜索 bot
│   ├── prompt-engineer/prompt-restructuring-guide.md → Prompt 重构 bot
│   ├── illustrator/image-generation-guide.md        → 图片生成 bot
│   ├── photo-regenerator/image-regeneration-guide.md → 图片修复 bot
│   └── archive/video-analyst/video-tagging-guide.md → 视频标注 bot（暂归档）
│
├── 📊 数据与产出
│   ├── rock-climbing-knowledge/src/data/
│   │   ├── kp-registry.json             → 支柱一：177 个知识点注册表（核心索引）
│   │   ├── training-registry.json       → 支柱二：训练方案注册表（待建立）
│   │   ├── athlete-registry.json        → 支柱三：运动员档案注册表（待建立）
│   │   ├── video-registry.json          → 视频标注数据
│   │   └── sections.json + section-01~10.json → 各模块详细内容
│   ├── prompt-engineer/                 → 288 条已拆分的图片生成 Prompt
│   ├── article-researcher/              → 文章搜索产出（JSON + 报告）
│   └── video-researcher/video-research-results.md → 视频搜索产出
│
└── 🗂️ archive/                          ← 历史参考文件
```
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `video-researcher/video-research-guide.md` |
| **任务** | 在 YouTube / Bilibili 搜索攀岩教学视频，匹配到知识点 |
| **输出** | `video-researcher/video-research-results.md` 及其子目录下的轮次结果 |
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `archive/video-analyst/video-tagging-guide.md` |
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `article-researcher/article-research-guide.md` |
| **任务** | 搜索攀岩相关高质量文章，映射到知识点，按模板输出 JSON |
| **输出** | `article-researcher/section-XX-articles.json` + `article-researcher/section-XX-report.md` |
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `prompt-engineer/prompt-restructuring-guide.md` |
| **任务** | 将复杂多子图 Prompt 拆分为独立 Prompt |
| **输出** | `prompt-engineer/module-XX-*.md` |
| **当前进度** | ✅ 已全部完成 |
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `illustrator/image-generation-guide.md` |
...
| 属性 | 说明 |
|------|------|
| **指导文件** | `photo-regenerator/image-regeneration-guide.md` |
...
**图片生成 Bot 审核要点**：
- [ ] 风格是否一致（线稿、绿+灰+白底）？
- [ ] 攀岩装备/动作画对了吗（参考 `prompt-engineer/prompt-restructuring-guide.md` 中的错误清单）？
- [ ] 标注文字是否清晰可读？
- [ ] 是否与 Prompt 描述匹配？
...
目前存在两套视频数据格式：
- `rock-climbing-knowledge/src/data/video-registry.json`（`archive/video-analyst/video-tagging-guide.md` 定义的格式）
- `video-researcher/video-research-results.md`（`video-researcher/video-research-guide.md` 定义的 markdown 表格格式）
...
### 查看文章搜索进度
→ 查看 `article-researcher/` 目录下已有的 section 文件

### 查看视频搜索进度
→ 读取 `video-researcher/video-research-results.md` + `video-researcher/reference/influencer-video-master-table.md`

### 查看 Prompt 拆分结果
→ 读取 `prompt-engineer/module-XX-*.md`

### 查看某个图片的原始 Prompt
→ 在 `prompt-engineer/` 目录下搜索 kp-id

### 查看知识体系全貌
→ 读取 `team-lead/climbing-knowledge-framework.md`
...
*最后更新：2026-03-06*
*项目规模：三支柱架构 × 177 个知识点（支柱一）× 7 个 Bot*
*注：任务完成后的文件整理规则已移至 `CLAUDE.md`，全局自动生效。*
