...
> **目标**：将 `reference/illustration-prompts.md` 中的复杂多子图 Prompt 拆分为**独立的、每张只描述一件事**的 Prompt，让图片生成 AI 能更聚焦，出图质量更高，同时保证所有产出图的**布局结构高度一致**。
...
### Step 1 — 阅读源文件
读取 `reference/illustration-prompts.md`，它包含约 173 张插图的 Prompt，按 10 个知识模块分组。
...
请按以下顺序处理 `reference/illustration-prompts.md` 中全部 173 条 Prompt：
